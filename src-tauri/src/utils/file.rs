...
pub async fn save_upload_file(
    ...
    if let Some(folder) = sub_folder {
        ...
        // 确保子目录存在
        if !file_path.exists() {
            fs::create_dir_all(&file_path).await.map_err(|e| e.to_string())?;
        }
    ...
    fs::write(&file_path, data).await.map_err(|e| format!("Write failed: {}", e))?;

    // 返回相对路径，例如 "products/550e8400...jpg"
    // 注意：我们假设最外层已经是 "uploads" 了，所以这里返回的是相对于 uploads 内部的路径
