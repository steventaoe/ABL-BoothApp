...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/vendor-default-password", put(update_vendor_default_password))
}
...
async fn update_admin_password(
    ...
    let stored_hash = match row {
        Some((h,)) => h,
        None => return (StatusCode::INTERNAL_SERVER_ERROR, "Admin password not set in database").into_response(),
    };

    // 2. 验证旧密码
    if !verify_password(&payload.old_password, &stored_hash) {
        return (StatusCode::UNAUTHORIZED, Json(json!({"error": "旧密码错误"}))).into_response();
    }
    ...
    match result {
        Ok(_) => (StatusCode::OK, Json(json!({"message": "管理员密码已更新"}))).into_response(),
...
async fn update_vendor_default_password(
    ...
    let result = sqlx::query("INSERT OR REPLACE INTO settings (key, value) VALUES ('vendor_password', ?)")
        .bind(new_hash)
        .execute(&state.db)
        .await;

    match result {
        Ok(_) => (StatusCode::OK, Json(json!({"message": "默认摊主密码已更新"}))).into_response(),
        Err(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Database Error").into_response(),
    }
}
